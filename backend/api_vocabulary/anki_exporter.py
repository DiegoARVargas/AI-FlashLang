...
def generate_apkg_for_user(user, deck_name=None, ids=None, allow_duplicates=False) -> tuple[str, str]:
    ...
    for word in user_words:
        ...
        word_audio_tag = f"[sound:{os.path.basename(source.audio_word.name)}]" if source.audio_word else ""
        sentence_audio_tag = f"[sound:{os.path.basename(source.audio_sentence.name)}]" if source.audio_sentence else ""
