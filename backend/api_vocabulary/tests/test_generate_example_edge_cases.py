...
class GenerateExampleEdgeCaseTests(APITestCase):
    def setUp(self):
        ...
        self.word = VocabularyWord.objects.create(word="testword", user=self.user)

    ''' Este test está desactivado temporalmente porque actualmente 
        no se permite editar palabras una vez creadas será reevaluado si se implementa edición futura.
    def test_generate_example_fails_if_already_has_example(self):
        #este metodoVerifica que si la palabra ya tiene un example_sentence, no se vuelva a generar 
        #y se responda con error o advertencia.
    ...
    def test_generate_example_invalid_id_returns_404(self):
        ...
        invalid_id = 9999
        url = reverse("vocabularyword-generate-example", args=[invalid_id])
        response = self.client.post(url)
